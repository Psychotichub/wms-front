...
const PasswordStrengthMeter = ({ password, colors, tr }) => {
  const getStrength = () => {
  ...
  const strengthLabels = ['', tr('login.strengthWeak'), tr('login.strengthFair'), tr('login.strengthGood'), tr('login.strengthStrong')];
...
const SignupScreen = ({ navigation }) => {
  ...
  const signupSchema = useMemo(
    () =>
      z.object({
        name: z.string().trim().min(1, tr('validation.nameRequired')),
        email: z.string().trim().min(1, tr('validation.emailRequired')).email(tr('validation.emailInvalid')),
        company: z.string().trim().min(1, tr('validation.companyRequired')),
        adminCode: z.string().optional(),
        password: z
          .string()
          .min(6, tr('validation.passwordMin'))
          .refine((val) => /[A-Z]/.test(val), {
            message: tr('validation.passwordUppercase')
          })
          .refine((val) => /[a-z]/.test(val), {
            message: tr('validation.passwordLowercase')
          })
          .refine((val) => /[0-9]/.test(val), {
            message: tr('validation.passwordNumber')
          })
          .refine((val) => /[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/.test(val), {
            message: tr('validation.passwordSpecial')
          })
      }),
    [tr]
  );
  const resolver = useMemo(() => buildZodResolver(signupSchema), [signupSchema]);
    ...
    <Screen>
      ...
      >
        ...
        <View style={styles.headerSection}>
          <Text style={[styles.title, { color: t.colors.primary }]}>{tr('signup.title')}</Text>
          <Text style={[styles.subtitle, { color: t.colors.textSecondary }]}>{tr('signup.subtitle')}</Text>
        </View>

        {/* Feature Highlights */}
        <View style={styles.featuresContainer}>
          <FeatureItem icon="location-outline" text={tr('login.featureTracking')} colors={t.colors} />
          <FeatureItem icon="people-outline" text={tr('login.featureTeam')} colors={t.colors} />
          <FeatureItem icon="stats-chart-outline" text={tr('login.featureAnalytics')} colors={t.colors} />
          <FeatureItem icon="phone-portrait-outline" text={tr('login.featureMobile')} colors={t.colors} />
        </View>

        {/* Trust Indicator */}
        <View style={styles.trustIndicator}>
          <Ionicons name="shield-checkmark" size={16} color={t.colors.success} />
          <Text style={[styles.trustText, { color: t.colors.textSecondary }]}>{tr('signup.secureSignup')}</Text>
        </View>
        ...
        >
              ...
              <View>
                <View style={[
                  ...
                  <TextInput
                    ...
                    placeholder={tr('signup.placeholderName')}
                    accessibilityLabel={tr('signup.placeholderName')}
                    value={value}
              ...
              <View>
                <View style={[
                  ...
                  <TextInput
                    ...
                    placeholder={tr('login.placeholderEmail')}
                    accessibilityLabel={tr('login.placeholderEmail')}
                    autoCapitalize="none"
              ...
              <View>
                <View style={[
                  ...
                  <TextInput
                    ...
                    placeholder={tr('login.placeholderCompany')}
                    accessibilityLabel={tr('login.placeholderCompany')}
                    value={value}
                    onChangeText={onChange}
              ...
              <View>
                <View style={[
                  ...
                  <TextInput
                    ...
                    placeholder={tr('signup.placeholderAdminCode')}
                    accessibilityLabel={tr('signup.placeholderAdminCode')}
                    value={value}
                    onChangeText={onChange}
              ...
              <View>
                <View style={[
                  ...
                  <TextInput
                    ...
                    accessibilityLabel={tr('login.placeholderPassword')}
                ...
                <PasswordStrengthMeter password={password} colors={t.colors} tr={tr} />
              </View>
            )}
          />
          ...
          <View style={styles.buttonContainer}>
            <Button
              title={isLoading ? tr('signup.creatingAccount') : tr('signup.submitCta')}
        ...
        <View style={styles.helpSection}>
          ...
          >
            ...
            <Text style={[styles.helpLinkText, { color: t.colors.primary }]}>{tr('login.needHelp')}</Text>
          </Pressable>
          ...
          >
            ...
            <Text style={[styles.helpLinkText, { color: t.colors.primary }]}>{tr('login.faq')}</Text>
          </Pressable>
        </View>
        ...
        {Platform.OS === 'web' && (
          <View style={styles.keyboardShortcuts}>
            <Text style={[styles.keyboardShortcutsText, { color: t.colors.textSecondary }]}>
              {tr('signup.enterToSignup')}{' '}
              <Text style={[styles.keyboardShortcutKey, { color: t.colors.text }]}>{tr('signup.enterToSignupKey')}</Text>{' '}
              {tr('signup.enterToSignupSuffix')}
            </Text>
          </View>
        )}
        ...
        <View style={styles.socialProof}>
          <Text style={[styles.socialProofText, { color: t.colors.textSecondary }]}>{tr('login.trustedBy')}</Text>
        </View>

        {/* Login Link */}
        <View style={styles.switchRow}>
          <Text style={{ color: t.colors.textSecondary }}>{tr('signup.alreadyHave')} </Text>
          <Pressable onPress={() => navigation.navigate('Login')}>
            <Text style={[styles.link, { color: t.colors.primary }]}>{tr('signup.loginLink')}</Text>
          </Pressable>
        </View>
      </ScrollView>
    </Screen>
