      ...
      <View style={styles.todoHeader}>
        ...
        <View style={styles.todoActions} pointerEvents="box-none">
          ...
          <Pressable
            ...
            onPress={() => {
              if (isLocked) {
                Alert.alert(tr('todos.cannotEditTitle'), tr('todos.cannotEditBody'), [{ text: tr('common.ok') }]);
              } else {
                onEdit(item);
              }
            }}
          ...
          <Pressable
            ...
            onPress={() => {
              if (isLocked) {
                Alert.alert(tr('todos.cannotDeleteTitle'), tr('todos.cannotDeleteBody'), [{ text: tr('common.ok') }]);
                return;
              }
              
              if (!onDelete) {
                Alert.alert(tr('common.error'), tr('todos.deleteUnavailable'));
                return;
              }
              
              if (!item._id) {
                Alert.alert(tr('common.error'), tr('todos.missingId'));
                return;
              }
...
const TodoListScreen = () => {
  ...
  const priorityOptions = useMemo(
    () => [
      { label: tr('todos.priorityLow'), value: 'low' },
      { label: tr('todos.priorityMedium'), value: 'medium' },
      { label: tr('todos.priorityHigh'), value: 'high' },
      { label: tr('todos.priorityUrgent'), value: 'urgent' }
    ],
    [tr]
  );
  ...
  const filterOptions = useMemo(
    () => [
      { label: tr('todos.filterAll'), value: 'all' },
      { label: tr('todos.filterActive'), value: 'active' },
      { label: tr('todos.filterCompleted'), value: 'completed' }
    ],
    [tr]
  );
  ...
  const loadTodos = useCallback(async () => {
    ...
    try {
      ...
      setMessage(error.message || tr('todos.loadFail'));
    } finally {
      setLoading(false);
    }
  }, [request, filter, searchValue, tr]);
  ...
  const handleSubmit = async () => {
    if (!formData.title.trim()) {
      setMessage(tr('todos.titleRequired'));
      return;
    }
    ...
    try {
      ...
      if (formData.reminder.enabled && formData.reminder.date && formData.reminder.time) {
        ...
        setMessage(tr('todos.reminderBoth'));
        return;
      }
      ...
      if (editingId) {
        ...
        setMessage(tr('todos.updated'));
      } else {
        await request('/api/todos', {
          method: 'POST',
          body: JSON.stringify(payload)
        });
        setMessage(tr('todos.created'));
      }

      resetForm();
      loadTodos();
    } catch (error) {
      setMessage(error.message || tr('todos.saveFail'));
    }
  };
  ...
  const handleToggle = useCallback(async (id) => {
    try {
      ...
      setMessage(error.message || tr('todos.toggleFail'));
    }
  ...
  const handleDelete = useCallback(async (id) => {
    ...
    if (isWeb) {
      ...
      return new Promise((resolve) => {
        Alert.alert(
          tr('todos.deleteConfirmTitle'),
          tr('todos.confirmDeleteWeb'),
          [
            { 
              text: tr('todos.cancel'), 
              style: 'cancel',
              onPress: () => {
                resolve();
              }
            },
            {
              ...
              onPress: async () => {
                try {
                  ...
                  setMessage(tr('todos.deleted'));
                  loadTodos();
                  resolve();
                } catch (error) {
                  setMessage(error.message || tr('todos.deleteFail'));
                  resolve();
                }
              }
            }
  ...
  const stats = useMemo(() => {
    return {
      ...
      withReminders: todos.filter((todo) => todo.reminder?.enabled && !todo.completed).length
    };
  }, [todos]);
