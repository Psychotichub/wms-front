  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!trimmedMaterial || !matchingMaterial) {
        setMessage(tr('dailyReport.pickMaterial'));
        return;
      }
      ...
      if (!trimmedLocation || !matchingLocation) {
        setMessage(tr('dailyReport.pickLocation'));
        return;
      }
      if (!date) {
        setMessage(tr('dailyReport.pickDate'));
        return;
      }
      const parsedDate = new Date(date);
      if (!(parsedDate instanceof Date) || Number.isNaN(parsedDate.getTime())) {
        setMessage(tr('dailyReport.invalidDate'));
        return;
      }
      ...
      if (trimmedPanel || trimmedCircuit) {
        ...
        if (!selectedPanel) {
          setMessage(tr('dailyReport.pickPanel'));
          return;
        }
        ...
        if (panelCircuits.length === 0) {
          setMessage(tr('dailyReport.noCircuits'));
          return;
        }
        if (!selectedCircuit) {
          setMessage(tr('dailyReport.pickCircuit'));
          return;
        }
      ...
      if (editingId) {
        ...
        if (offline) {
          ...
          resetForm();
          setMessage(tr('auth.queuedOffline'));
          return;
        }
