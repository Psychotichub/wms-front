...
const ContractQuantityScreen = () => {
  ...
  const loadContracts = useCallback(async () => {
    ...
    try {
      ...
      setMessage(err.message || tr('contracts.loadFail'));
    } finally {
      setLoading(false);
    }
  }, [request, hasSite, tr]);
  ...
  const handleSubmit = async () => {
    if (!materialId) {
      setMessage(tr('contracts.pickMaterial'));
      return;
    }
    const qty = Number(contractQuantity);
    if (!Number.isFinite(qty) || qty <= 0) {
      setMessage(tr('contracts.invalidQty'));
      return;
    }
    ...
    try {
      if (editingId) {
        ...
        setMessage(tr('contracts.updated'));
      } else {
  ...
  const performDelete = useCallback(async (id) => {
    ...
    try {
      ...
      setMessage(tr('contracts.deleted'));
      await loadContracts();
    } catch (err) {
      console.error('Delete error:', err);
      setMessage(err.message || tr('contracts.deleteFail'));
    }
  }, [request, loadContracts, tr]);

  const handleDelete = useCallback((id) => {
    if (!id) {
      console.error('Delete called with no ID');
      setMessage(tr('contracts.noId'));
      return;
    }

    if (Platform.OS === 'web') {
      // On web, use window.confirm for better compatibility
      const confirmed = window.confirm(tr('contracts.deleteConfirmWeb'));
      if (confirmed) {
        performDelete(id);
      }
    } else {
  ...
  return (
    <Screen>
      <View style={styles.container}>
        <Text style={[styles.title, { color: t.colors.text }]}>{tr('contracts.qtyTitle')}</Text>

        {isAdmin && (
          <>
            <Text style={[styles.label, { color: t.colors.textSecondary }]}>
              {editingId ? tr('contracts.editSection') : tr('contracts.addSection')}
            </Text>
            <Text style={[styles.label, { color: t.colors.textSecondary }]}>{tr('contracts.materialName')}</Text>
            <AutocompleteInput
              ...
              onBlur={(currentVal) => {
                if (!isInList(currentVal, materialNames)) {
                  setMaterialName('');
                  setMaterialId(null);
                  setMessage(tr('contracts.selectFromDropdown'));
                }
              }}
              placeholder={tr('contracts.selectMaterial')}
              containerStyle={{ marginBottom: 12, zIndex: 10000, elevation: 10 }}
            />

            <Text style={[styles.label, { color: t.colors.textSecondary }]}>{tr('contracts.totalQty')}</Text>
            <TextInput
              ...
              placeholder={tr('contracts.qtyPlaceholder')}
            ...
            <Button
              ...
              title={editingId ? tr('contracts.updateContract') : tr('contracts.addContract')}
        ...
        <View style={styles.tableContainer}>
          <View style={[styles.searchContainer, { borderColor: t.colors.border, backgroundColor: t.colors.card }]}>
            ...
            <TextInput
              ...
              placeholder={tr('contracts.searchPlaceholder')}
              value={searchValue}
                ...
                ListEmptyComponent={
                  !loading ? (
                    <EmptyState
                      ...
                      title={tr('contracts.emptyTitle')}
                      subtitle={searchValue ? tr('contracts.emptySubtitleSearch') : tr('contracts.emptySubtitle')}
                    />
                  ) : null
                }
                renderItem={({ item }) => (
