  ...
  const loginSchema = useMemo(
    () =>
      z.object({
        ...
        password: z.string().min(6, tr('validation.passwordMin'))
      }),
    [tr]
  );
  const resolver = useMemo(() => buildZodResolver(loginSchema), [loginSchema]);
    ...
    <Screen>
      ...
      >
        ...
        <View style={styles.headerSection}>
          <Text style={[styles.title, { color: t.colors.primary }]}>{tr('login.welcomeTitle')}</Text>
          <Text style={[styles.subtitle, { color: t.colors.textSecondary }]}>{tr('login.welcomeSubtitle')}</Text>
        </View>
        ...
        <View style={styles.trustIndicator}>
          ...
          <Text style={[styles.trustText, { color: t.colors.textSecondary }]}>{tr('login.secureLogin')}</Text>
        </View>
        ...
        >
          ...
          <View style={styles.optionsRow}>
            <Pressable
              ...
              accessibilityLabel={tr('login.rememberMeA11y')}
            >
              <View style={[
                styles.checkbox,
              ...
              <Text style={[styles.rememberMeText, { color: t.colors.textSecondary }]}>{tr('login.rememberMe')}</Text>
            </Pressable>
            <Pressable onPress={() => {/* TODO: Implement forgot password */}}>
              <Text style={[styles.forgotPassword, { color: t.colors.primary }]}>{tr('login.forgotPassword')}</Text>
            </Pressable>
          </View>

          <View style={styles.buttonContainer}>
            <Button
              title={isLoading ? tr('login.loggingIn') : tr('login.loginCta')}
              onPress={handleSubmit(onSubmit)}
              disabled={isLoading || !isValid}
              accessibilityLabel={isLoading ? tr('login.loggingInA11y') : tr('login.loginCtaA11y')}
            />
            {isLoading && (
              <Animated.View
                style={[
        ...
        {Platform.OS === 'web' && (
          <View style={styles.keyboardShortcuts}>
            <Text style={[styles.keyboardShortcutsText, { color: t.colors.textSecondary }]}>
              {tr('login.enterToLogin')}{' '}
              <Text style={[styles.keyboardShortcutKey, { color: t.colors.text }]}>{tr('login.enterToLoginKey')}</Text>{' '}
              {tr('login.enterToLoginSuffix')}
            </Text>
          </View>
        )}
        ...
        <View style={styles.switchRow}>
          <Text style={{ color: t.colors.textSecondary }}>{tr('login.newToWms')} </Text>
          <Pressable onPress={() => navigation.navigate('Signup')}>
            <Text style={[styles.link, { color: t.colors.primary }]}>{tr('login.createAccount')}</Text>
          </Pressable>
        </View>
      </ScrollView>
    </Screen>
