  ...
  return (
    ...
    >
        ...
        <View style={styles.cardTitleContainer}>
          ...
          {isCurrent && (
            <View style={styles.currentBadge}>
              ...
              <Text style={styles.currentBadgeText}>{tr('locations.select.currentBadge')}</Text>
            </View>
          )}
        </View>
      ...
      <Text style={[styles.cardAddress, { color: colors.textSecondary }]}>
        📍 {geofence.address || tr('locations.select.noAddress')}
      </Text>
      ...
      <View style={styles.cardStatus}>
        <View style={styles.statusContainer}>
          ...
          <Text style={[
            ...
            {isCheckedInHere
              ? tr('locations.select.statusCheckedIn')
              : isCurrent
                ? tr('locations.select.statusAtLocation')
                : tr('locations.select.statusAvailable')}
          </Text>
        </View>
...
const LocationSelectionScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    ...
    loadGeofences().catch((error) => {
      ...
      setGeofenceError(error.message || tr('locations.select.loadFail'));
    });

    // Start location tracking when screen opens with error handling
    startLocationTracking().catch((error) => {
      console.error('Failed to start location tracking:', error);
      setLocationError(error.message || tr('locations.select.permFail'));
    });
  ...
  const handleSaveWorkingHours = async () => {
    ...
    setTimeout(async () => {
      try {
        ...
        /* LocationContext will reconcile when GPS is ready */
      }
    }, 300);
  };
  ...
  const renderGeofenceItem = useCallback(
    ...
    [attendanceStatus.elapsedTime, attendanceStatus.isCheckedIn, currentGeofence?.id, handleGeofenceSelect, selectedGeofence?.id, t.colors, tr]
  );
  ...
  const handleRefreshLocation = async () => {
    try {
      ...
      if (!isTracking) {
        ...
        setTimeout(() => {
          startLocationTracking().catch((error) => {
            setLocationError(error.message || tr('locations.select.trackStartFail'));
          });
        }, 1000);
      }
    } catch (error) {
      console.error('Error refreshing location:', error);
      setLocationError(error.message || tr('locations.select.refreshFail'));
    }
  };
  ...
  const formatTime = (date) => {
    if (!date) return tr('locations.select.never');
      ...
      <View style={styles.container}>
        ...
        <View style={[styles.header, { backgroundColor: t.colors.card, borderBottomColor: t.colors.border }]}>
          ...
          <View style={styles.headerCenter}>
            <Text style={[styles.headerTitle, { color: t.colors.text }]}>{tr('locations.select.header')}</Text>
            {currentGeofence && (
              <Text style={[styles.headerSubtitle, { color: t.colors.textSecondary }]}>
                {tr('locations.select.currentPrefix', { name: currentGeofence.name })}
              </Text>
            )}
          </View>
        ...
        <View style={[styles.statusBar, { backgroundColor: t.colors.card, borderBottomColor: t.colors.border, borderBottomWidth: 1 }]}>
          <View style={styles.statusItem}>
            ...
            <Text style={[styles.statusText, { color: t.colors.text }]}>
              {locationPermission
                ? tr('locations.select.locActive')
                : locationError
                  ? tr('locations.select.locError')
                  : tr('locations.select.locDisabled')}
            </Text>
          </View>
          ...
          <View style={styles.statusItem}>
            ...
            <Text style={[styles.statusText, { color: t.colors.text }]}>
              {attendanceStatus.isCheckedIn ? formatElapsedTime(attendanceStatus.elapsedTime) : tr('locations.select.notCheckedIn')}
            </Text>
          </View>
        </View>
          ...
          {(() => {
            ...
            {location && (
              <Marker
                ...
                description={tr('geofenceMap.accuracyM', { m: String(Math.round(location.coords.accuracy)) })}
            ...
            {geofences.map((geofence) => {
              ...
              <View style={[styles.mapPlaceholder, { backgroundColor: 'transparent' }]}>
                <EmptyState
                  ...
                  subtitle={locationError || tr('locations.select.emptyMapSubtitle')}
                />
              </View>
            );
          })()}
          ...
          {geofenceError ? (
            <View style={styles.errorCard}>
              ...
              >
                ...
                <Text style={styles.retryButtonText}>{tr('locations.select.retry')}</Text>
              </Pressable>
            </View>
          ) : (
            <FlatList
              ...
              ListEmptyComponent={
                <View style={styles.emptyCard}>
                  <EmptyState
                    ...
                    title={tr('locations.select.emptyConfiguredTitle')}
                    subtitle={tr('locations.select.emptyConfiguredSubtitle')}
                  />
                </View>
              }
            />
          ...
          <View style={[styles.actionPanel, { backgroundColor: t.colors.card, borderTopColor: t.colors.border }]}>
            <View style={styles.actionInfo}>
              <Text style={[styles.selectedLocation, { color: t.colors.text }]}>
                {tr('locations.select.selectedPrefix', { name: selectedGeofence.name })}
              </Text>
              <View style={styles.workingHoursRow}>
                <View style={styles.workingHoursInfo}>
                  ...
                  <Text style={[styles.workingHoursText, { color: t.colors.text }]}>
                    {tr('locations.select.workingHoursLine', {
                      start: formatTimeForDisplay(workingHours.startTime),
                      end: formatTimeForDisplay(workingHours.endTime)
                    })}
                  </Text>
                </View>
                <Pressable
              ...
              <Text style={[styles.lastActivity, { color: t.colors.textSecondary }]}>
                {tr('locations.select.lastCheckIn', { time: formatTime(lastCheckInTime) })}
              </Text>
              <Text style={[styles.lastActivity, { color: t.colors.textSecondary }]}>
                {tr('locations.select.lastCheckOut', { time: formatTime(lastCheckOutTime) })}
              </Text>
              <Text style={[styles.lastActivity, { color: t.colors.textSecondary }]}>
                {tr('locations.select.autoCheckHint')}
              </Text>
            </View>
          </View>
        )}
          ...
          <View style={styles.modalOverlay}>
            <View style={[styles.modalContent, { backgroundColor: t.colors.card, borderColor: t.colors.border }]}>
              <View style={styles.modalHeader}>
                <Text style={[styles.modalTitle, { color: t.colors.text }]}>{tr('locations.select.setHours')}</Text>
              ...
              <TimeInput
                label={tr('locations.select.labelStartTime')}
                value={tempStartTime}
              ...
              <TimeInput
                label={tr('locations.select.labelEndTime')}
                value={tempEndTime}
              ...
              <Text style={[styles.modalHint, { color: t.colors.textSecondary }]}>
                {tr('locations.select.modalTrackingHint')}
              </Text>
              ...
              <View style={styles.modalActions}>
                ...
                >
                  <Text style={[styles.modalButtonText, { color: t.colors.text }]}>{tr('locations.select.cancel')}</Text>
                </Pressable>
                <Pressable
                  style={[
                    styles.modalButton,
                ...
                >
                  <Text style={[styles.modalButtonText, { color: '#ffffff' }]}>{tr('locations.select.save')}</Text>
                </Pressable>
              </View>
            </View>
          </View>
