...
const InventoryScreen = () => {
  ...
  const loadInventory = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const data = await request('/api/inventory?limit=2000');
          ...
          <View style={[styles.table, { borderColor: t.colors.border }]}>
              <FlatList
                ...
                ListHeaderComponent={
                  ...
                  >
                    <Text style={[styles.cell, styles.headerCell, styles.materialCell, { color: t.colors.text }]}>{tr('inventory.colMaterial')}</Text>
                    <Text style={[styles.cell, styles.headerCell, { color: t.colors.text }]}>{tr('inventory.colReceived')}</Text>
                    <Text style={[styles.cell, styles.headerCell, { color: t.colors.text }]}>{tr('inventory.colConsumption')}</Text>
                    <Text style={[styles.cell, styles.headerCell, { color: t.colors.text }]}>{tr('inventory.colStock')}</Text>
                    <Text style={[styles.cell, styles.headerCell, styles.statusCell, { color: t.colors.text }]}>{tr('inventory.colStatus')}</Text>
                  </View>
                }
                ListEmptyComponent={
                  !loading ? (
                    <EmptyState
                      icon="cube-outline"
                      title={tr('inventory.emptyTitle')}
                      subtitle={searchValue ? tr('inventory.emptySubtitleSearch') : tr('inventory.emptySubtitleNoData')}
                    />
                  ) : null
                }
                renderItem={({ item }) => (
