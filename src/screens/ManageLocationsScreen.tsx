...
import GeofenceMap, { Marker, Polygon, Circle } from '../components/GeofenceMap';
...
const ManageLocationsScreen = () => {
  ...
  const { loadGeofences, location: currentLocation, getCurrentLocation } = useLocation();
  ...
  // Google Places Autocomplete service (web only)
  const autocompleteServiceRef = useRef(null);
  const placesServiceRef = useRef(null);
  
  // Initialize Google Places services when Maps API is loaded (web only)
  useEffect(() => {
    ...
    const initializeServices = () => {
      if (window.google?.maps?.places) {
        ...
        if (!placesServiceRef.current) {
          // PlacesService needs a map or div element, we'll create a dummy one
    ...
    // If not ready, wait for the API to load
    if (!window.google?.maps?.places) {
  ...
  const buttonLabel = useMemo(
    () => (loading ? tr('locations.manage.saveSaving') : tr('locations.manage.save')),
    [loading, tr]
  );
    ...
    try {
      ...
      if (!locationToUse?.coords) {
        try {
          ...
          console.error('[ManageLocationsScreen] Failed to get current location:', error);
          Alert.alert(
            tr('locations.manage.locUnavailableTitle'),
            error.message || tr('locations.manage.locUnavailableBody'),
            [{ text: tr('common.ok') }]
          );
          return;
        }
      ...
      if (!locationToUse?.coords) {
        console.warn('[ManageLocationsScreen] Location still not available after request');
        Alert.alert(
          tr('locations.manage.locUnavailableTitle'),
          tr('locations.manage.locServicesBody'),
          [{ text: tr('common.ok') }]
        );
        return;
      }
      ...
      console.error('[ManageLocationsScreen] Error in goToCurrentLocation:', error);
      Alert.alert(
        tr('common.error'),
        error.message || tr('locations.manage.locGenericError'),
        [{ text: tr('common.ok') }]
      );
    }
  };
  ...
  const fetchPlaces = async (query) => {
    ...
    // On web, use Google Maps JS API Places Autocomplete service
    if (Platform.OS === 'web') {
      if (!window.google?.maps?.places) {
      ...
      try {
        autocompleteServiceRef.current.getPlacePredictions(
          ...
          (predictions, status) => {
            if (status === window.google.maps.places.PlacesServiceStatus.OK && predictions) {
              // Map predictions to match the expected format
              ...
              if (status === window.google.maps.places.PlacesServiceStatus.REQUEST_DENIED) {
                Alert.alert(tr('locations.manage.titleApi'), tr('locations.manage.apiErrorPlacesBullets'));
              }
              setSearchResults([]);
            }
          }
        ...
        Alert.alert(tr('locations.manage.titleNetwork'), tr('locations.manage.networkSearch'));
        setSearchResults([]);
      }
      return;
    }
    
    // On native, fall back to REST API (if needed in the future)
    if (!googleKey) {
      console.error('Google Maps API key is missing');
      Alert.alert(tr('locations.manage.titleConfig'), tr('locations.manage.configMaps'));
      return;
    }
    try {
      ...
      if (data.error_message) {
        ...
        Alert.alert(tr('common.error'), tr('locations.manage.searchError', { message: data.error_message }));
        setSearchResults([]);
        return;
      }
  ...
  const handleSearchSelect = useCallback(async (place) => {
    ...
    // On web, use Google Maps JS API Places Service
    if (Platform.OS === 'web') {
      if (!window.google?.maps?.places) {
        Alert.alert(tr('common.error'), tr('locations.manage.placesNotLoaded'));
        return;
      }
      ...
      try {
        placesServiceRef.current.getDetails(
          ...
          (result, status) => {
            if (status === window.google.maps.places.PlacesServiceStatus.OK && result) {
              ...
              const placeName = result.name || place.description;
              ...
              if (loc) {
                ...
                if (lat && lng) {
                  setSearchMarker({
                    latitude: lat,
                    longitude: lng,
                    title: placeName || tr('locations.manage.selectedPlace'),
                  });
                  if (isPolygon) {
                  ...
                  Alert.alert(tr('common.error'), tr('locations.manage.placeDetailsFail'));
                }
              } else {
                console.error('No location found for place:', place);
                Alert.alert(tr('common.error'), tr('locations.manage.placeDetailsFail'));
              ...
              console.error('Places Details API error:', status);
              if (status === window.google.maps.places.PlacesServiceStatus.REQUEST_DENIED) {
                Alert.alert(tr('locations.manage.titleApi'), tr('locations.manage.placeDetailsDenied'));
              } else {
                Alert.alert(tr('common.error'), tr('locations.manage.placeDetailsStatus', { status: String(status) }));
              }
            }
          }
        );
        ...
        Alert.alert(tr('locations.manage.titleNetwork'), tr('locations.manage.networkDetails'));
      }
      return;
    }
    ...
    try {
      ...
      if (data.error_message) {
        ...
        Alert.alert(tr('common.error'), tr('locations.manage.placeDetailsStatus', { status: data.error_message }));
        return;
      }
  ...
  const submit = async () => {
    if (!token) {
      Alert.alert(tr('locations.manage.loginAlertTitle'), tr('locations.manage.loginRequired'));
      return;
    }

    if (!name.trim()) {
      Alert.alert(tr('locations.manage.validationAlertTitle'), tr('locations.manage.validationName'));
      return;
    }
    ...
    if (isPolygon) {
      ...
      if (coords.length < 3) {
        Alert.alert(tr('locations.manage.validationAlertTitle'), tr('locations.manage.polygonMin'));
        return;
      }
      ...
      if (!Number.isFinite(lon) || !Number.isFinite(lat)) {
        Alert.alert(tr('locations.manage.validationAlertTitle'), tr('locations.manage.circleCenter'));
        return;
      }
      if (!Number.isFinite(rad) || rad <= 0) {
        Alert.alert(tr('locations.manage.validationAlertTitle'), tr('locations.manage.radiusPositive'));
        return;
      }
    ...
    try {
      ...
      Alert.alert(tr('locations.manage.success'), data?.message || (editingId ? tr('locations.manage.updated') : tr('locations.manage.saved')));
      setName('');
      setAddress('');
      setCoordinatesText('');
      setCenterLon('');
      ...
      Alert.alert(tr('common.error'), err.message || (editingId ? tr('locations.manage.updateError') : tr('locations.manage.saveError')));
    } finally {
      setLoading(false);
    }
  };
  ...
  const formatDate = useCallback(
    (dateString) => {
      if (!dateString) return tr('attendance.na');
  ...
  const formatCoordinates = useCallback(
    (location) => {
      if (location.type === 'circle' && location.center && location.radius) {
        return tr('locations.manage.coordCenterRadius', {
          lon: location.center[0].toFixed(6),
          lat: location.center[1].toFixed(6),
          radius: String(location.radius)
        });
      }
      if (location.coordinates && location.coordinates.length > 0) {
        return tr('locations.manage.coordPointCount', { count: location.coordinates.length });
      }
      return tr('locations.manage.noCoords');
    },
    [tr]
  );
  ...
  const performDelete = useCallback(async (locationId) => {
    if (!locationId) {
      ...
      Alert.alert(tr('common.error'), tr('locations.manage.invalidId'));
      return;
    }
    ...
    try {
      ...
      Alert.alert(tr('common.error'), err.message || tr('locations.manage.deleteError'));
    }
  }, [request, loadLocations, loadGeofences, editingId, tr]);
  ...
  const handleDelete = useCallback((locationId) => {
    ...
    if (Platform.OS === 'web') {
      ...
      // On native, use Alert
      Alert.alert(
        tr('locations.manage.deleteTitle'),
        tr('locations.manage.deleteConfirm'),
        [
          { 
  ...
  const renderLocationItem = useCallback(({ item }) => (
    <View style={[styles.locationCard, { backgroundColor: t.colors.card, borderColor: t.colors.border }]}>
      <View style={styles.locationHeader}>
        <View style={styles.locationTitleRow}>
          ...
          <View style={[styles.statusBadge, { backgroundColor: item.isActive ? getRGBA('#22c55e', 0.15) : getRGBA(t.colors.textSecondary, 0.15) }]}>
            <Text style={[styles.statusText, { color: item.isActive ? '#22c55e' : t.colors.textSecondary }]}>
              {item.isActive ? tr('locations.manage.active') : tr('locations.manage.inactive')}
            </Text>
          </View>
        </View>
        <View style={styles.locationActions}>
          <View style={styles.typeBadge}>
            ...
            />
            <Text style={[styles.typeText, { color: t.colors.textSecondary }]}>
              {item.type === 'circle' ? tr('locations.manage.circle') : tr('locations.manage.polygon')}
            </Text>
          </View>
            ...
            <Pressable
              ...
              onPress={() => {
                ...
                if (locationId) {
                  ...
                  Alert.alert(tr('common.error'), tr('locations.manage.idNotFound'));
                }
              }}
              hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}
            >
      ...
      <View style={styles.locationInfoRow}>
        ...
        <Text style={[styles.locationInfo, { color: t.colors.textSecondary }]}>
          {tr('locations.manage.createdAt', { date: formatDate(item.createdAt) })}
        </Text>
      </View>
      ...
      {item.createdBy && (
        <View style={styles.locationInfoRow}>
          ...
          <Text style={[styles.locationInfo, { color: t.colors.textSecondary }]}>
            {tr('locations.manage.byCreator', {
              name: item.createdBy.name || item.createdBy.email || tr('locations.manage.unknownCreator')
            })}
          </Text>
        </View>
      )}
    </View>
  ), [t.colors, formatCoordinates, formatDate, handleEdit, handleDelete, tr]);
  ...
  return (
    <Screen>
      <View style={[styles.header, { backgroundColor: t.colors.card, borderBottomColor: t.colors.border }]}>
        ...
        <Text style={[styles.headerTitle, { color: t.colors.text }]}>{tr('locations.manage.header')}</Text>
      ...
      >
        ...
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            <Text style={[styles.sectionTitle, { color: t.colors.text }]}>{tr('locations.manage.allLocations')}</Text>
            <Pressable
          ...
          {loadingLocations ? (
            <View style={styles.loadingContainer}>
              <Text style={[styles.loadingText, { color: t.colors.textSecondary }]}>{tr('locations.manage.loadingList')}</Text>
            </View>
          ) : locations.length === 0 ? (
            <View style={styles.emptyContainer}>
              <Ionicons name="location-outline" size={48} color={t.colors.textSecondary} />
              <Text style={[styles.emptyText, { color: t.colors.textSecondary }]}>{tr('locations.manage.emptyNone')}</Text>
              <Text style={[styles.emptySubtext, { color: t.colors.textSecondary }]}>{tr('locations.manage.emptyHint')}</Text>
            </View>
          ) : (
            <View>
        ...
        <Text style={[styles.title, { color: t.colors.text }]}>
          {editingId ? tr('locations.manage.editTitle') : tr('locations.manage.createTitle')}
        </Text>
        <Text style={[styles.subtitle, { color: t.colors.textSecondary }]}>{tr('locations.manage.formSubtitle')}</Text>

        <View style={styles.row}>
          <Text style={[styles.label, { color: t.colors.text }]}>{tr('locations.manage.nameLabel')}</Text>
          <TextInput
            value={name}
            onChangeText={setName}
            placeholder={tr('locations.manage.namePlaceholder')}
        ...
        <View style={styles.row}>
          <Text style={[styles.label, { color: t.colors.text }]}>{tr('locations.manage.typeLabel')}</Text>
          <View style={styles.typeRow}>
            ...
            >
              ...
              <Text style={[styles.typeText, { color: !isPolygon ? '#0b1220' : t.colors.text }]}>
                {tr('locations.manage.circle')}
              </Text>
            </Pressable>
          </View>
        </View>
        ...
        <View style={styles.row}>
          <Text style={[styles.label, { color: t.colors.text }]}>
            {tr('locations.manage.mapTapAdd', {
              hint: isPolygon ? tr('locations.manage.mapTapPolygon') : tr('locations.manage.mapTapCircle')
            })}
          </Text>
          <View style={styles.mapCard}>
            ...
            >
              {currentLocation?.coords && (
                <Marker
                  ...
                  title={tr('geofenceMap.yourLocation')}
                  description={tr('geofenceMap.accuracyM', {
                    m: String(Math.round(currentLocation.coords.accuracy || 0))
                  })}
                  pinColor="#ef4444"
                />
              )}
              ...
              {isPolygon &&
                parsedCoords.map((coord, idx) => (
                  <Marker
                    ...
                    title={tr('geofenceMap.markerPoint', { n: String(idx + 1) })}
                  />
                ))}
              ...
              {!isPolygon && Number.isFinite(parseFloat(centerLat)) && Number.isFinite(parseFloat(centerLon)) && (
                <>
                  {Number.isFinite(parseFloat(radius)) && parseFloat(radius) > 0 && (
                    <Circle
                      center={{
                        latitude: parseFloat(centerLat),
                        longitude: parseFloat(centerLon)
                  ...
                  <Marker
                    ...
                    title={tr('geofenceMap.markerCenter')}
                    description={radius ? tr('geofenceMap.radiusM', { m: String(radius) }) : undefined}
                    pinColor="#3b82f6"
                  />
                </>
              )}
            ...
            <View style={[styles.searchContainer, { backgroundColor: t.colors.card, borderColor: t.colors.border }]}>
              ...
              <TextInput
                ...
                placeholder={tr('locations.manage.searchPlaces')}
                placeholderTextColor={t.colors.textSecondary}
            ...
            {isPolygon && parsedCoords.length > 0 && (
              <Pressable style={[styles.clearBtn, { borderColor: t.colors.border }]} onPress={clearPolygon}>
                ...
                <Text style={[styles.clearText, { color: t.colors.text }]}>{tr('locations.manage.clearPolygon')}</Text>
              </Pressable>
            )}
          </View>
        </View>
        ...
        {isPolygon ? (
          <View style={styles.row}>
            <Text style={[styles.label, { color: t.colors.text }]}>{tr('locations.manage.coordsLabel')}</Text>
            <TextInput
          ...
          <>
            <View style={styles.row}>
              ...
              <View style={styles.centerRow}>
                <TextInput
                  ...
                  placeholder={tr('locations.manage.placeholderLon')}
                  placeholderTextColor={t.colors.textSecondary}
                ...
                <TextInput
                  ...
                  placeholder={tr('locations.manage.placeholderLat')}
                  placeholderTextColor={t.colors.textSecondary}
                  keyboardType="numeric"
                  style={[styles.inputHalf, { color: t.colors.text, borderColor: t.colors.border }]}
                />
            ...
            <View style={styles.row}>
              <Text style={[styles.label, { color: t.colors.text }]}>{tr('locations.manage.radiusMetersLabel')}</Text>
              <TextInput
                value={radius}
                onChangeText={setRadius}
                placeholder={tr('locations.manage.radiusExample')}
                placeholderTextColor={t.colors.textSecondary}
                keyboardType="numeric"
                style={[styles.input, { color: t.colors.text, borderColor: t.colors.border }]}
              />
        ...
        <View style={styles.submitRow}>
          {editingId && (
            ...
            >
              ...
              <Text style={[styles.cancelText, { color: t.colors.text }]}>{tr('common.cancel')}</Text>
            </Pressable>
          )}
          <Pressable
