...
const notificationTypeLabel = (tr, type) => {
  const path = `notifications.types.${type}`;
  const label = tr(path);
  if (label === path) return tr('notifications.types.default');
  return label;
};
...
const NotificationItem = React.memo(({ item, colors, onPress }) => {
  ...
  const formatTimeAgo = (date) => {
    ...
    if (diffInSeconds < 60) return tr('notifications.timeJustNow');
    if (diffInSeconds < 3600) return tr('notifications.timeMinutesAgo', { n: Math.floor(diffInSeconds / 60) });
    if (diffInSeconds < 86400) return tr('notifications.timeHoursAgo', { n: Math.floor(diffInSeconds / 3600) });
    if (diffInSeconds < 604800) return tr('notifications.timeDaysAgo', { n: Math.floor(diffInSeconds / 86400) });

    return notificationDate.toLocaleDateString(localeTag);
  };
  if (item.__skeleton) {
    return (
      <View
      ...
      <View style={styles.notificationContent}>
        ...
        <View style={styles.notificationMeta}>
          <Text style={[styles.notificationType, {
            ...
            {notificationTypeLabel(tr, item.type).toUpperCase()}
          </Text>
        ...
        {item.sender && (
          <Text style={[styles.notificationSender, { color: colors.textSecondary }]}>
            {tr('notifications.fromSender', { name: item.sender.name })}
          </Text>
        )}
      </View>
...
const NotificationsScreen = () => {
  ...
  const handleMarkAllRead = () => {
    ...
    if (isWeb) {
      const shouldMarkAll = window.confirm(tr('notifications.confirmMarkAllWeb'));
      if (shouldMarkAll) {
        markAllAsRead();
      }
    } else {
      ...
      Alert.alert(
        tr('notifications.markAllAlertTitle'),
        tr('notifications.markAllAlertMessage'),
        [
          { text: tr('common.cancel'), style: 'cancel' },
          {
            text: tr('notifications.markAllReadAction'),
          ...
          {notifications.length > 0 && (
            ...
            >
              <Text style={[styles.markAllText, { color: t.colors.text }]}>
                {tr('notifications.markAllReadBtn')}
              </Text>
            </Pressable>
          )}
        </View>
          ...
          ListEmptyComponent={
            !isLoading ? (
              <EmptyState
                ...
                title={tr('notifications.emptyListTitle')}
                subtitle={tr('notifications.emptyListSubtitle')}
              />
            ) : null
          }
        />
