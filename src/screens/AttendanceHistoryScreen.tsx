...
const AttendanceItem = React.memo(({ item, colors, tr, locale }) => {
  ...
  const formatItemTime = (dateString) => {
  ...
  const formatDuration = (hours) => {
    if (!hours) return tr('attendance.durationFormat', { h: 0, m: 0 });
    const h = Math.floor(hours);
    const m = Math.round((hours - h) * 60);
    return tr('attendance.durationFormat', { h, m });
  };
  ...
  <View style={[styles.attendanceItem, { backgroundColor: colors.card, borderColor: colors.border }]}>
    <View style={styles.itemHeader}>
      ...
      {item.location?.geofenceTriggered && (
        <View style={styles.geofenceBadge}>
          ...
          <Text style={styles.geofenceText}>{tr('attendance.auto')}</Text>
        </View>
      )}
    </View>
    ...
    <View style={styles.timeContainer}>
      <View style={styles.timeBlock}>
        ...
        <Text style={[styles.timeText, { color: colors.text }]}>
          {formatItemTime(item.clockInTime)}
        </Text>
      </View>
      ...
      <View style={styles.timeBlock}>
        ...
        <Text style={[styles.timeText, { color: colors.text }]}>
          {formatItemTime(item.clockOutTime)}
        </Text>
      </View>
    </View>
    ...
    <View style={styles.itemFooter}>
      <Text style={[styles.locationText, { color: colors.textSecondary }]}>
        📍 {item.location?.locationName || item.location?.name || tr('attendance.unknownLocation')}
      </Text>
...
const AttendanceHistoryScreen = ({ navigation, route }) => {
  ...
  const loadAttendanceHistory = useCallback(async () => {
    try {
      ...
      Alert.alert(tr('attendance.error'), tr('attendance.historyLoadFail'));
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  }, [getDateRange, request, selectedDateRange, selectedLocation, tr]);
  ...
  const exportToCSV = async () => {
    try {
      if (filteredHistory.length === 0) {
        Alert.alert(tr('attendance.exportNoDataTitle'), tr('attendance.exportNoDataBody'));
        return;
      }

      // Create CSV content
      const headers = [
        tr('attendance.csvDate'),
        tr('attendance.csvCheckIn'),
        tr('attendance.csvCheckOut'),
        tr('attendance.csvDurationH'),
        tr('attendance.csvLocation'),
        tr('attendance.csvGeofence')
      ];
      const csvContent = [
        headers.join(','),
        ...filteredHistory.map(record => [
          new Date(record.date).toLocaleDateString(),
          record.clockInTime ? new Date(record.clockInTime).toLocaleTimeString() : tr('attendance.na'),
          record.clockOutTime ? new Date(record.clockOutTime).toLocaleTimeString() : tr('attendance.na'),
          record.totalHours?.toFixed(2) || '0.00',
          `"${record.location?.locationName || record.location?.name || tr('attendance.unknownLocation')}"`,
          record.location?.geofenceTriggered ? tr('attendance.csvYes') : tr('attendance.csvNo')
        ].join(','))
      ].join('\n');
      ...
      if (await Sharing.isAvailableAsync()) {
        await Sharing.shareAsync(fileUri, {
          mimeType: 'text/csv',
          dialogTitle: tr('attendance.exportDialogTitle')
        });
      } else {
        Alert.alert(tr('attendance.success'), tr('attendance.exportSaved', { path: fileUri }));
      }
    } catch (error) {
  ...
  const renderAttendanceItem = useCallback(
    ({ item }) => <AttendanceItem item={item} colors={t.colors} tr={tr} locale={locale} />,
    [t.colors, tr, locale]
  );
  ...
  const renderLocationFilterButton = (location) => (
    ...
    >
      <Text style={[
        ...
        {location === 'all' ? tr('attendance.filterAllLocations') : location}
      </Text>
    </Pressable>
  );
    ...
    <Screen>
      <View style={[styles.container, { backgroundColor: t.colors.background }]}>
        ...
        <View style={[styles.header, { backgroundColor: t.colors.card, borderBottomColor: t.colors.border }]}>
          ...
          <Text style={[styles.headerTitle, { color: t.colors.text }]}>{tr('attendance.historyTitle')}</Text>
          <Pressable
        ...
        <View style={[styles.statsContainer, { backgroundColor: t.colors.card }]}>
          <View style={styles.statItem}>
            ...
            <Text style={[styles.statLabel, { color: t.colors.textSecondary }]}>{tr('attendance.statsDays')}</Text>
          </View>
          <View style={styles.statItem}>
            <Text style={[styles.statValue, { color: t.colors.primary }]}>{stats.totalHours}h</Text>
            <Text style={[styles.statLabel, { color: t.colors.textSecondary }]}>{tr('attendance.statsTotalHours')}</Text>
          </View>
          <View style={styles.statItem}>
            <Text style={[styles.statValue, { color: t.colors.primary }]}>{stats.averageHoursPerDay}h</Text>
            <Text style={[styles.statLabel, { color: t.colors.textSecondary }]}>{tr('attendance.statsAvgDay')}</Text>
          </View>
        </View>
        ...
        <View style={[styles.filtersContainer, { backgroundColor: t.colors.background }]}>
          ...
          <View style={styles.filterSection}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.filterScroll}>
              {renderFilterButton('week', tr('attendance.filterWeekBtn'))}
              {renderFilterButton('month', tr('attendance.filterMonthBtn'))}
              {renderFilterButton('custom', tr('attendance.filterCustomBtn'))}
            </ScrollView>
          </View>
          ...
          {availableLocations.length > 1 && (
            <View style={styles.filterSection}>
              <Text style={[styles.filterTitle, { color: t.colors.text }]}>{tr('attendance.filterLocation')}</Text>
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.filterScroll}>
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <EmptyState
                ...
                title={tr('attendance.emptyTitle')}
                subtitle={tr('attendance.emptyFiltersHint')}
              />
            }
          />
        )}
