...
import { StyleSheet, Text, TextInput, View, Pressable, FlatList, Platform } from 'react-native';
...
const SKELETON_DATA = Array.from({ length: 5 }).map((_, idx) => ({
  id: `material-skeleton-${idx}`,
  __skeleton: true,
}));
...
const MaterialRow = React.memo(({ item, colors, onEdit, onDelete }) => {
  ...
  return (
    ...
    prevProps.item.materialPrice === nextProps.item.materialPrice &&
    prevProps.item.labourPrice === nextProps.item.labourPrice &&
...
const AddMaterialScreen = () => {
  ...
  return (
    <Screen>
      ...
      <Text style={[styles.title, { color: t.colors.text }]}>{tr('materials.addTitle')}</Text>
      <Text style={[styles.label, { color: t.colors.textSecondary }]}>{tr('materials.labelName')}</Text>
      <AutocompleteInput
        data={Array.from(new Set(materials.map(m => m.name)))}
        value={name}
        onChange={setName}
        onSelect={(item) => setName(item.label)}
        placeholder={tr('materials.namePlaceholder')}
        containerStyle={{ marginBottom: 12 }}
      />
      <Text style={[styles.label, { color: t.colors.textSecondary }]}>{tr('materials.labelUnit')}</Text>
      ...
      />
      <Text style={[styles.label, { color: t.colors.textSecondary }]}>{tr('materials.labelLabourPrice')}</Text>
      <TextInput
        style={[styles.input, { borderColor: t.colors.border, color: t.colors.text, backgroundColor: t.colors.card }]}
        placeholder={tr('materials.labourPlaceholder')}
        keyboardType="numeric"
        value={labourPrice}
      ...
      <Button title={editingId ? tr('materials.updateMaterial') : tr('materials.saveMaterial')} onPress={handleSubmit} />
      {message ? <Text style={[styles.message, { color: t.colors.text }]}>{message}</Text> : null}
      {loading || materials.length > 0 ? (
        <>
          <Text style={[styles.label, { color: t.colors.textSecondary }]}>{tr('materials.labelSearch')}</Text>
          <AutocompleteInput
            data={materials.map(m => m.name)}
            value={searchValue}
            onChange={setSearchValue}
            onSelect={(item) => setSearchValue(item.label)}
            placeholder={tr('materials.searchPlaceholder')}
            containerStyle={{ marginBottom: 12 }}
          />
          ...
          <View style={[styles.table, { borderColor: t.colors.border }]}>
            ...
            >
              <Text style={[styles.cell, styles.headerCell, { color: t.colors.text }]}>{tr('materials.colName')}</Text>
              <Text style={[styles.cell, styles.headerCell, { color: t.colors.text }]}>{tr('materials.colUnit')}</Text>
              <Text style={[styles.cell, styles.headerCell, { color: t.colors.text }]}>{tr('materials.colMaterialPrice')}</Text>
              <Text style={[styles.cell, styles.headerCell, { color: t.colors.text }]}>{tr('materials.colLabourPrice')}</Text>
              <Text style={[styles.cell, styles.headerCell, { color: t.colors.text }]}>{tr('materials.colTotalPrice')}</Text>
              <Text style={[styles.cell, styles.headerCell, { color: t.colors.text }]}>{tr('materials.colAction')}</Text>
            </View>
            <FlatList
              data={loading ? SKELETON_DATA : filteredMaterials}
          ...
          {!loading && filteredMaterials.length === 0 ? (
            <EmptyState
              icon="cube-outline"
              title={tr('materials.emptySearchTitle')}
              subtitle={tr('materials.emptySearchSubtitle')}
            />
          ) : null}
        </>
      ) : (
        !loading && (
          <EmptyState
            icon="cube-outline"
            title={tr('materials.emptyListTitle')}
            subtitle={tr('materials.emptyListSubtitle')}
          />
        )
      )}
      </View>
