...
const WorkTrendsChart = ({ data, color, labelColor, backgroundColor, tasksWord }) => {
...
const DashboardScreen = ({ navigation }) => {
  ...
  const stats = useMemo(
    () => [
      { label: tr('dashboard.statReports'), value: counts.reports, icon: 'document-text-outline', color: t.colors.success },
      { label: tr('dashboard.statMaterials'), value: counts.materials, icon: 'cube-outline', color: t.colors.primary },
      { label: tr('dashboard.statPanels'), value: counts.panels, icon: 'git-branch-outline', color: t.colors.warning },
      {
        label: tr('dashboard.statActiveEmployees'),
        value: analytics.productivityKPIs.activeEmployees,
        icon: 'people-outline',
        color: t.colors.info
      }
    ],
    [counts, analytics, t, tr]
  );
    ...
    <Screen>
      <View style={styles.header}>
        <View style={styles.headerContent}>
          <View>
            <Text style={[styles.title, { color: t.colors.text }]}>{tr('dashboard.welcomeBack')}</Text>
            <Text style={[styles.subtitle, { color: t.colors.textSecondary }]}>
              {user?.name || tr('dashboard.teamMember')} · {user?.company || tr('dashboard.company')} ·{' '}
              {user?.site || tr('dashboard.site')}
            </Text>
          </View>
          <Pressable
            onPress={() => setShowCustomizationModal(true)}
            accessibilityRole="button"
            accessibilityLabel={tr('dashboard.customizeA11y')}
            hitSlop={8}
      ...
      {widgets.overview && (
        <View style={styles.section}>
          <Text style={[styles.sectionTitle, { color: t.colors.text }]}>{tr('dashboard.overview')}</Text>
          {loading ? <Text style={[styles.muted, { color: t.colors.textSecondary }]}>{tr('dashboard.refreshing')}</Text> : null}
      ...
      {widgets.productivityKPIs && (
        <View style={styles.section}>
        <Text style={[styles.sectionTitle, { color: t.colors.text }]}>{tr('dashboard.productivityKpis')}</Text>
          ...
          {loading ? (
            ...
            <>
              <View style={[styles.kpiCard, { backgroundColor: t.colors.card, borderColor: t.colors.border }]}>
                ...
                <Text style={[styles.kpiLabel, { color: t.colors.textSecondary }]}>{tr('dashboard.kpiTotalTasks')}</Text>
              </View>
              <View style={[styles.kpiCard, { backgroundColor: t.colors.card, borderColor: t.colors.border }]}>
                ...
                <Text style={[styles.kpiLabel, { color: t.colors.textSecondary }]}>{tr('dashboard.kpiAvgEfficiency')}</Text>
              </View>
              <View style={[styles.kpiCard, { backgroundColor: t.colors.card, borderColor: t.colors.border }]}>
                ...
                <Text style={[styles.kpiLabel, { color: t.colors.textSecondary }]}>{tr('dashboard.kpiTotalHours')}</Text>
              </View>
            </>
          )}
        </View>
      ...
      {widgets.workTrends && (
        <View style={styles.section}>
        <Text style={[styles.sectionTitle, { color: t.colors.text }]}>{tr('dashboard.workTrendsTitle')}</Text>
        <Text style={[styles.chartSubtitle, { color: t.colors.textSecondary }]}>{tr('dashboard.workTrendsSubtitle')}</Text>
        {loading ? (
          ...
          <View style={[styles.chartWrapper, { backgroundColor: t.colors.card, borderColor: t.colors.border }]}>
            <WorkTrendsChart
              ...
              tasksWord={tr('dashboard.chartTasksSuffix')}
            />
          </View>
        ) : (
          <EmptyState
            icon="stats-chart-outline"
            title={tr('dashboard.noTrendTitle')}
            subtitle={tr('dashboard.noTrendSubtitle')}
          />
        )}
      </View>
      )}
      ...
      {widgets.topPerformers && (
        ...
        {loading ? (
              ...
              >
                ...
                <View style={styles.employeeInfo}>
                  ...
                  <Text style={[styles.employeeRole, { color: t.colors.textSecondary }]}>
                    {employee.role} · {employee.productivityMetrics?.tasksCompleted || 0} {tr('dashboard.tasksWord')}
                  </Text>
                </View>
          ...
          <EmptyState
            icon="trophy-outline"
            title={tr('dashboard.noPerformersTitle')}
            subtitle={tr('dashboard.noPerformersSubtitle')}
          />
        )}
      </View>
      )}
      ...
      {widgets.quickActions && (
        <View style={styles.section}>
        <Text style={[styles.sectionTitle, { color: t.colors.text }]}>{tr('dashboard.quickActions')}</Text>
        <View style={styles.actionRow}>
          <Pressable
            ...
            accessibilityRole="button"
            accessibilityLabel={tr('dashboard.a11yDailyReport')}
          >
            <Ionicons name="document-text-outline" size={20} color={t.colors.primary} />
            <Text style={[styles.actionText, { color: t.colors.text }]}>{tr('dashboard.actionDailyReport')}</Text>
          </Pressable>
          <Pressable
            style={[styles.actionCard, elevation.medium, { backgroundColor: t.colors.card, borderColor: t.colors.border }]}
            onPress={() => navigation.navigate('Add Material')}
            accessibilityRole="button"
            accessibilityLabel={tr('dashboard.a11yAddMaterial')}
          >
            <Ionicons name="add-circle-outline" size={20} color={t.colors.success} />
            <Text style={[styles.actionText, { color: t.colors.text }]}>{tr('dashboard.actionAddMaterial')}</Text>
          </Pressable>
        </View>
        <View style={styles.actionRow}>
          <Pressable
            ...
            accessibilityRole="button"
            accessibilityLabel={tr('dashboard.a11yPanels')}
          >
            <Ionicons name="git-branch-outline" size={20} color={t.colors.warning} />
            <Text style={[styles.actionText, { color: t.colors.text }]}>{tr('dashboard.actionPanels')}</Text>
          </Pressable>
          {user?.role === 'admin' && (
            <Pressable
              style={[styles.actionCard, elevation.medium, { backgroundColor: t.colors.card, borderColor: t.colors.border }]}
              ...
              accessibilityRole="button"
              accessibilityLabel={tr('dashboard.a11yEmployees')}
            >
              <Ionicons name="people-outline" size={20} color={t.colors.info} />
              <Text style={[styles.actionText, { color: t.colors.text }]}>{tr('dashboard.actionEmployees')}</Text>
            </Pressable>
          )}
        </View>
        <View style={styles.actionRow}>
          <Pressable
            ...
            accessibilityRole="button"
            accessibilityLabel={tr('dashboard.a11ySettings')}
          >
            <Ionicons name="settings-outline" size={20} color={t.colors.primary} />
            <Text style={[styles.actionText, { color: t.colors.text }]}>{tr('dashboard.actionSettings')}</Text>
          </Pressable>
        </View>
      </View>
      )}
