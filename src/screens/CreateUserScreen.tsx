...
const CreateUserScreen = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setRole('user');
      setCompany('');
      setMessage(tr('createUser.created'));
      loadUsers();
    } catch (err) {
      setMessage(err.message);
    }
  ...
  const handleUpdatePassword = async () => {
    ...
    try {
      ...
      setMessage(tr('createUser.passwordUpdated', { name: selectedUser.name }));
  ...
  const handleDeleteUser = useCallback(async (userItem) => {
    ...
    const userName = userItem.name || userItem.email || tr('createUser.deleteConfirmFallback');
    const isWeb = Platform.OS === 'web';
    
    // Show confirmation dialog
    if (isWeb) {
      const shouldDelete = window.confirm(tr('createUser.deleteMessage', { name: userName }));
      if (!shouldDelete) {
        return;
      }
      ...
      try {
        ...
        setMessage(tr('createUser.deleted'));
        loadUsers();
      } catch (err) {
        setMessage(err.message);
      }
      ...
      return new Promise((resolve) => {
        Alert.alert(
          tr('createUser.deleteTitle'),
          tr('createUser.deleteMessage', { name: userName }),
          [
            { 
              text: tr('createUser.cancel'), 
              style: 'cancel',
              onPress: () => {
                resolve();
              }
  ...
  return (
    <Screen>
      <View style={styles.container}>
        ...
        <Text style={[styles.title, { color: t.colors.text }]}>{tr('createUser.title')}</Text>
        <TextInput
          style={[styles.input, { borderColor: t.colors.border, color: t.colors.text, backgroundColor: t.colors.card }]}
          placeholder={tr('createUser.placeholderName')}
          value={name}
        ...
        <TextInput
          style={[styles.input, { borderColor: t.colors.border, color: t.colors.text, backgroundColor: t.colors.card }]}
          placeholder={tr('createUser.placeholderEmail')}
          autoCapitalize="none"
        ...
        <TextInput
          style={[styles.input, { borderColor: t.colors.border, color: t.colors.text, backgroundColor: t.colors.card }]}
          placeholder={tr('createUser.placeholderPassword')}
          secureTextEntry
          value={password}
        ...
        <TextInput
          style={[styles.input, { borderColor: t.colors.border, color: t.colors.text, backgroundColor: t.colors.card }]}
          placeholder={tr('createUser.placeholderCompany')}
          value={company}
        ...
        <TextInput
          style={[styles.input, { borderColor: t.colors.border, color: t.colors.text, backgroundColor: t.colors.card }]}
          placeholder={tr('createUser.placeholderRole')}
          value={role}
          onChangeText={setRole}
          placeholderTextColor={t.colors.textSecondary}
        />
        <Button title={tr('createUser.submit')} onPress={handleSubmit} />

        {message ? <Text style={[styles.message, { color: t.colors.text }]}>{message}</Text> : null}

        <Text style={[styles.tableTitle, { color: t.colors.text, marginTop: 32 }]}>{tr('createUser.tableTitle')}</Text>
        ...
        <View style={[styles.table, { borderColor: t.colors.border }]}>
          <View style={[styles.row, styles.headerRow, { backgroundColor: t.colors.card, borderColor: t.colors.border }]}>
            <Text style={[styles.cell, styles.headerCell, { color: t.colors.text, flex: 2 }]}>{tr('createUser.colUser')}</Text>
            <Text style={[styles.cell, styles.headerCell, { color: t.colors.text }]}>{tr('createUser.colRole')}</Text>
        ...
        {!loading && users.length === 0 ? (
          <EmptyState
            ...
            title={tr('createUser.emptyTitle')}
            subtitle={tr('createUser.emptySubtitle')}
          />
        ) : null}

        {selectedUser && (
          <View style={[styles.modal, { backgroundColor: t.colors.card, borderColor: t.colors.primary }]}>
            <Text style={[styles.modalTitle, { color: t.colors.text }]}>{tr('createUser.resetTitle', { name: selectedUser.name })}</Text>
            <TextInput
              style={[styles.input, { borderColor: t.colors.border, color: t.colors.text, backgroundColor: t.colors.background }]}
              placeholder={tr('createUser.placeholderNewPassword')}
              secureTextEntry
              value={newPassword}
            ...
            <View style={{ flexDirection: 'row', gap: 8 }}>
              <View style={{ flex: 1 }}>
                <Button title={tr('createUser.update')} onPress={handleUpdatePassword} />
              </View>
              <View style={{ flex: 1 }}>
                <Button title={tr('createUser.cancel')} variant="secondary" onPress={() => setSelectedUser(null)} />
              </View>
            </View>
          </View>
        )}
