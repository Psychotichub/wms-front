...
const TaskRow = React.memo(({ item, colors, onEdit, onViewDetails, tr, isKanban, columnWidth }) => {
  if (item.__skeleton) {
    return (
    ...
    <View style={[styles.taskCard, { borderColor: colors.border, backgroundColor: colors.card, padding: cardPadding }]}>
      ...
      <View style={[styles.taskMeta, { gap: metaGap }]}>
        <View style={styles.taskMetaItem}>
          <Ionicons name="person-outline" size={metaIconSize} color={colors.textSecondary} />
      ...
      <View style={[styles.taskActions, { flexDirection: 'row', gap: 6, flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center' }]}>
        <Pressable
          style={[styles.viewDetailsBtn, { backgroundColor: getRGBA(colors.primary, 0.1), borderColor: colors.primary, paddingVertical: btnPaddingVer, paddingHorizontal: btnPaddingHor }]}
          onPress={() => onViewDetails(item._id)}
        >
          <Ionicons name="eye-outline" size={isKanban ? 13 : 16} color={colors.primary} />
          {showBtnText && <Text style={[styles.viewDetailsText, { color: colors.primary, fontSize: btnFontSize }]}>View Details</Text>}
        </Pressable>
        {item.status !== 'completed' && (
          <Pressable
            style={[styles.actionBtn, { backgroundColor: getRGBA(colors.primary, 0.12), padding: isKanban ? 6 : 8 }]}
            onPress={() => onEdit(item)}
            hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}
          >
            <Ionicons name="create-outline" size={isKanban ? 14 : 18} color={colors.primary} />
          </Pressable>
        )}
      </View>
    </View>
...
const TaskScreen = () => {
  ...
  const kanbanColumns = useMemo(() => [
    ...
    { key: 'cancelled', label: tr('tasks.statusCancelled'), color: t.colors.statusCancelled }
  ], [tr, t.colors]);
  ...
  ), [t.colors, handleEdit, handleViewDetails, tr, windowWidth]);
